...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        encode,(last_hidden,last_cell) = self.encoder(X_packed)
        enc_hiddens, lengths = pad_packed_sequence(encode)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        o_prev = torch.zeros(Y.size()[1],Y.size()[2])
        Y = torch.split(Y,1)
        combined_outputs = []
        laststate = dec_init_state
        for yt in Y:
            yt = torch.squeeze(yt)
            ybart = torch.cat((yt,o_prev),1)
            dec_state,o_t,e_t = self.step(ybart,laststate,enc_hiddens,enc_hiddens_proj,enc_masks)
            laststate = dec_state
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ###
        ###       Hints:
        ###         - dec_hidden is shape (b,h,1) and corresponds to h^dec_t in the PDF (batched)
